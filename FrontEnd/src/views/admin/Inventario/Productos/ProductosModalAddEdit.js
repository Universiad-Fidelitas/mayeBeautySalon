...
export const ProductosModalAddEdit = ({ tableInstance, apiParms }) => {
    ...
    const categoryDataDropdown = useMemo(() => categoriesData?.items.map(({ category_id, name }) => {
      return { value: category_id, label: name };
    }), [categoriesData]);

    const brandDataDropdown = useMemo(() => brandsData?.items.map(({ brand_id, name }) => {
      return { value: brand_id, label: name };
    }), [brandsData]);

    const providerDataDropdown = useMemo(() => providersData?.items.map(({ provider_id, name, email }) => {
                ...
                <Row className="g-3 mb-3">
                  ...
                  <Col className="col-4">
                    <div className="top-label">
                      ...
                      <Field value={parseInt(values.size, 10) ? parseInt(values.size, 10) : ''} 
                        onChange={({ target }) => setFieldValue('size', `${target.value}${values.size.replace(parseInt(values.size, 10), '')}`)}
                        className={`form-control ${errors.size && touched.size ? 'is-invalid' : ''}`} id="size" name="size" />
                    </div>
                  </Col>
                  <Col className="col-4">
                    <div className="top-label">
                      <label className="form-label bg-transparent">{f({ id: 'products.totalAmount' })}</label>
                      <Field className="form-control" id="size" name="size" value={`${parseInt(values.size, 10) ? parseInt(values.size, 10) : 0} ${values.size.replace(parseInt(values.size, 10), '')}`} disabled />
                    </div>
                  </Col>
