...
export const ModalAddEditUsuarios = ({ tableInstance, addItem, validationSchema, formFields }) => {
  ...
  const rolDataDropdown = useMemo(() => rolesData?.items.map(({ role_id, name }) => {
  ...
  return (
    <Modal className="modal-right" show={isOpenAddEditModal} onHide={() => setIsOpenAddEditModal(false)}>
      <Card className={classNames('mb-5', { 'overlay-spinner': isLoading })}>
        <Formik initialValues={selectedFlatRows.length === 1 ? selectedFlatRows[0].values : initialData} onSubmit={onSubmit} validationSchema={validationSchema}>
          <Form>
            <Modal.Header>
            ...
            <Modal.Body>
              ...
              <div className="mb-3">
                ...
                <Select classNamePrefix="react-select" options={rolDataDropdown} value={userRolSelected} onChange={setUserRolSelected} placeholder="Seleccione el rol del usuario" />
